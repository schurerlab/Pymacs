...
def automate_pdb2gmx(directory, polymer_info=None, allow_missing_atoms=False):
    ...
    command = (
        f"gmx pdb2gmx -f {os.path.basename(pdb2gmx_input_path)} -o protein_processed.gro "
        f"-p topol.top -ignh -water spc -ff {forcefield_name} -ter{missing_flag}"
    )
...
def prepare_component_topology(directory, component_code, pdb_filename, role, molecule_count, source_pdb_path=None):
    ...
    if paramchem:
        ...
        if not run_command(
            f"gmx editconf -f {os.path.basename(ini_pdb_file)} -o {os.path.basename(single_gro_file)}",
            cwd=directory,
        ):
            return None
    else:
        ...
        if not run_command(
            f"gmx editconf -f {os.path.basename(pose_final)} -o {os.path.basename(single_gro_file)}",
            cwd=directory,
        ):
            return None
    ...
    if os.path.exists(user_all_gro) and os.path.getsize(user_all_gro) > 0:
        ...
        if not run_command(
            f"gmx editconf -f {os.path.basename(all_pdb_file)} -o {os.path.basename(all_gro_file)}",
            cwd=directory,
        ):
            return None
